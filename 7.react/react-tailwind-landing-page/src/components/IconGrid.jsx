...
const IconGrid = ({ className = "" }) => {
    return (
        <ul className={`flex items-center justify-between ${className}`}>
            {BRAINWAVE_SERVICE_ICONS.map((item, index) => (
                <li
                    key={index}
                ...
                >
                    <div
                        className={
                            index === 2
                                ? "flex z-10 items-center justify-center w-full bg-neutral-2/80 h-full rounded-2xl transition-colors duration-300 hover:bg-neutral-2/90"
                                : ""
                        }
                    >
                        <img
